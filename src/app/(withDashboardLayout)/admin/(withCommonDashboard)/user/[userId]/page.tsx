"use client";
import { useSingleUserProfileDataQuery } from "@/components/Redux/UserApi/userApi";
...
const UserProfileDetail: React.FC<UserProfileDetailProps> = ({ params }) => {
  const profileId = params.userId;
  const { data, isLoading } = useSingleUserProfileDataQuery(profileId);
      ...
      <div className=" bg-gray-100 h-[100vh] w-[90%] mx-auto -mt-24 p-12 rounded-2xl ">
        <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12">
          ...
          <div className=" space-y-1">
            <h1 className="text-xl primaryColor font-bold bg-gray-200 px-4 p-1">
              {profileData?.name || "N/A"}
            </h1>
            <h2 className="text-xl text-gray-600 bg-gray-200 px-4 p-1">
              {profileData?.email || "N/A"}
            </h2>
            <h3 className="text-lg text-gray-500 bg-gray-200 px-4 p-1">
